...
- Require a final documentation audit only for material-risk work: new or updated dependencies, persistence or migrations, security, privacy, authentication, external integrations, deployment or operations changes, or when the user explicitly requests an audit. For ordinary UI or Client features, focused implementation and test evidence plus a concise final summary are sufficient completion evidence.
